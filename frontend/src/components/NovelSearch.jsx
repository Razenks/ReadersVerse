...
function NovelsBySearch() {
    ...
    useEffect(() => {
        if (searchQuery.trim()) {
            axios
                .get(`/api/novels/search/${encodeURIComponent(searchQuery)}`)
                .then((res) => setNovels(res.data))
                .catch((err) => console.error('Erro ao buscar novels por nome:', err));
        }
    }, [searchQuery]);

    return (
        <main className="max-w-5xl mx-auto px-4 py-6">
            <h1 className="text-xl font-bold mb-8">Results for "{searchQuery}"</h1>

            <div className="h-[3px] w-full bg-gradient-to-r from-black via-gray-800 to-gray-600 mb-10" />

            {novels.length === 0 ? (
                <p className="text-gray-600">No novels found.</p>
            ) : (
                        ...
                        return (
                            <Link key={novel.id} to={`/novel/${novel.id}`} className="flex w-[400px]">
                                ...
                                <div className="flex flex-col justify-between w-full overflow-hidden">
                                    ...
                                    <p className="mb-1 text-sm text-left flex">
                                        <BookOpen className="w-5 h-5 mr-1" />
                                        {novel.chapter_count || 0} Chapters
                                    </p>
                                    <p className="text-sm">
